# MailboxValidator Node.js API


```{py:function} MailboxValidator_init(api_key)
Configure MailboxValidator API key.

:param str api_key: (Required) MailboxValidator API key.
```

```{py:function} MailboxValidator_single_query(email)
...
```{py:function} MailboxValidator_disposable_email(email)
...
```{py:function} MailboxValidator_free_email(email)
